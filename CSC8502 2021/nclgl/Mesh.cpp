...
void Mesh::Draw() {
	...
	if(Mesh::isWireFrame) glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
...
void Mesh::randomVertexColours()
{
	if (counter > 20) {
		counter = 0;
		glBindVertexArray(arrayObject);
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[COLOUR_BUFFER]);
		Vector4* cols = (Vector4*)glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
		for (int i = 0; i < numVertices; ++i) {
			cols[i] = Vector4(static_cast <float> (rand()) / static_cast <float> (RAND_MAX), static_cast <float> (rand()) / static_cast <float> (RAND_MAX), static_cast <float> (rand()) / static_cast <float> (RAND_MAX), 1.0f);
		}
